...
public:
  FrontierCostmap() {
    ...
    cm_pub = n.advertise<nav_msgs::OccupancyGrid>(costmap_topic, 1000);
    ...
    n.getParam("frontier_cm_attentiveness", attentiveness);
  ...
  void map_callback(const nav_msgs::OccupancyGrid & cm) {
    ...
    for (int i = 0; i < int(cm.data.size()); i = i + attentiveness) {
        if (cm.data[i] == 0 && is_frontier(temp, i)) weights.push_back(100);
